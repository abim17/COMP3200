    ...
    if (status == google.maps.places.PlacesServiceStatus.OK) {
      ...
      for (var i = 0; i < 3; i++) {
        if (typeof results[i] !== 'undefined') {
          // places[i,1] = results[i].formatted_address;
          // places[i,2] = results[i].geometry.location.lat;
          // places[i,3] = results[i].geometry.location.lng;
          // places[i,4] = results[i].icon;
          // places[i,5] = results[i].name;
          // places[i,6] = results[i].rating;
          // places[i,7] = results[i].reference;
        
          html += '<div class="col-md-4"><img class="fa fa-heart-o" src="' 
          + results[i].icon 
          + '"><h4>' 
          + results[i].name 
          + '</h4><dl class="left wordwrap dl-horizontal"><dt>Rating</dt><dd>This attraction has been rated '
          +results[i].rating+' out of a possible 5.</dd> <dt>Address</dt><dd>'
          +results[i].formatted_address+'</dd><dt>Type</dt><dd>'
          +results[i].types+'</dd><dt>Photos</dt><dd>'
